	...
	try
	{
		...
		*/
		...
		/*	VxSmartInterface<IExtension> dynamicsCableExtension;
			...
			solverPar->setConstraintStdSolverTolerance(0.000001);
...
//
			...
			Vx::VxVector3 targetV = (*tetheredTargetPartPtr)->getVxPart()->getLinearVelocity();
			...
			while ((application->update()) && (currentTime < finalTime))
			{
				...
				//if (currentTime > 10.0 && SwitchSimTime == 0)
				//{
				//	SwitchSimTime = 1;
				//	tstep = 0.001;
				//	application->setSimulationFrameRate(1 / tstep);
...
//#endif
				...
				VxVector3 ChaserTorqueInput = SlidingModeChaserAttControl_ST(chaserPartPtr, partVector[0], VxVector3(1, 1, 1), VxVector3(4, 4, 4), VxVector3(0, 0, 0), double(.001), outSMC);
				(*chaserPartPtr)->addTorque(ChaserTorqueInput);
				...
				Vx::VxVector3 Eye = CenterOfMass_System + 70 * CoMUV;//
