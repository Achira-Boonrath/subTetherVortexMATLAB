...
void attachDistanceN_ST(VxSmartInterface<Assembly> assembly, VxSmartInterface<Part> chaser, VxSmartInterface<Part> target, VxSmartInterface<Part>* partMatrix, VxReal MaxDistance, VxReal threadStiffness, VxReal threadDamping, int numNode, float partMass)
{
	...
	//threadStiffness = 1568.0;
	threadStiffness = 3136.0;
	...
	for (int i = 0; i < numNode + 4; i++)
	{
		if (i == 0)
		{
			...
			//attachDistance(assembly, chaser, partMatrix[i], VxVector3(-0.5, 0, 0), VxVector3(0, 0, 0), MaxDistance, threadStiffness, threadDamping);
		...
		else if (i == numNode)
		{
			...
			//attachDistance(assembly, partMatrix[numNode - 1], target, VxVector3(0, 0, 0), VxVector3(-11.75 / 2.0, -1.25 / 2.0, -1.25 / 2.0), MaxDistance, threadStiffnessST, threadDamping);
			//attachDistance(assembly, partMatrix[numNode - 1], target, VxVector3(0, 0, 0), VxVector3(-1.25 / 2.0, -1.25 / 2.0, -11.25 / 2.0), MaxDistance, threadStiffnessST, threadDamping);
