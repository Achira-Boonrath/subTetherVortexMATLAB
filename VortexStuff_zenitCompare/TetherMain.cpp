...
#include <VxDynamics/ConstraintOutputAttachmentContainer.h>
...
#include <VxDynamics/Spring.h>
#include <VxDynamics/SpringICD.h>
...
int main(int argc, const char *argv[])
{
    ...
    try 
    {
		...
		VxReal sphereRadius = 0.1; 
        sphere->parameterRadius = sphereRadius ;
		part1->parameterMassPropertiesContainer.mass = 0.0001;
        ...
        VxVector3 pos2 = VxVector3(0, 0, -L0);
		...
		(*attachmentPoint1)[PointDefinitionContainerID::kPointTypeID].setValue(VxEnum(PointDefinitionType::kAttachmentPoint));
		(*attachmentPoint1)[PointDefinitionContainerID::kPartID].setValue(part1.getExtension()); 
		//(*attachmentPoint1)[PointDefinitionContainerID::kOffsetID].setValue(Vx::VxVector3(0.0, 0.0, -chaserSideLength/2.0));
		...
		//(*attachmentPoint2)[PointDefinitionContainerID::kOffsetID].setValue(Vx::VxVector3(0.0, 0.0, sphereRadius));
		...
		params[CableSystemParamDefinitionContainerID::kCollisionGeometryTypeID].setValue(1); // 0: None, 1: spheres, 2: capsule
		params[CableSystemParamDefinitionContainerID::kLinearDensityID].setValue(0.000000001); // kg/m
		...
		params[CableSystemParamDefinitionContainerID::kBendingDampingID].setValue(EIused/10);    
		...
		segmentDefinition0[SegmentDefinitionContainerID::kAttachmentTypeStartID].setValue(1); // 0: ball/socket 1:cantilever
		...
		segmentDefinition0[SegmentDefinitionContainerID::kDragCoefficientID].setValue(0.000000001);
		segmentDefinition0[SegmentDefinitionContainerID::kFlexibleID].setValue(true);    
		segmentDefinition0[SegmentDefinitionContainerID::kCollisionGeometryTypeID].setValue(1);
		...
		//segmentDefinition0[SegmentDefinitionContainerID::kBuoyancyForceScaleID].setValue(0.0000001);
		...
		VxVector3 pos = part2->getVxPart()->getPosition();
		VxVector3 vel = part2->getVxPart()->getLinearVelocity();
        ...
        // Run the simulation
		 // Not a member of VxSim::VxApplication ?? 
        ...
        {    
			...
			fprintf(outVZ, "\n");
			fprintf(outF, "\n");
			VxReal3 externalForceTip = { 0.0,10.0, 0.0 };
			//if (currentTime>0)
			part2->getVxPart()->addForce(externalForceTip);
		}
