function ZenitOut = ZenitRotated(Zenit,Rmat)

    % Rotates Zenit based on the rotation matrix Rmat. At the moment, make
    % sure all the body components are composed of the same number of
    % polygons N.
    ZenitOut = Zenit;
    ...
    RotPos = Rmat*([X1;Y1;Z1]); %rotation about center of mass
    ZenitOut.states.quat = rotm2quat(Rmat*Zenit.body.rotationMatrix);
    ZenitOut.body.rotationMatrix = Rmat*Zenit.body.rotationMatrix;
    %figure; hold on;
    for ii = 1:6
      
        X = [RotPos(1,2*N*(ii-1)+1:N*(2*ii-1));RotPos(1,(2*ii-1)*N+1:2*N*ii)];
        Y = [RotPos(2,2*N*(ii-1)+1:N*(2*ii-1));RotPos(2,(2*ii-1)*N+1:2*N*ii)];
        Z = [RotPos(3,2*N*(ii-1)+1:N*(2*ii-1));RotPos(3,(2*ii-1)*N+1:2*N*ii)];
