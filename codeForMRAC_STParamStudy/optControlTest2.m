...
% The states are propagated with ode45 from initial condition using the control decision variables.
    ...
    function J = objFun(U)
        % COST FUNCTION
        % We iterate through the time steps, integrating the physics to find
        % the state trajectory resulting from the current control guess 'U'.

        % Approximate integral using trapezoid rule for u^2
        J_run = dt * sum(U.^2);
        J_run = R_u * J_run;

        % Propagate states to get terminal penalty
        x = xd0;
        for k = 1:N
            u_k = U(k);
    ...
    function [c, ceq] = nonlcon(U)
        ...
        % We define boundary conditions here. Since we shoot forward from a
        % fixed initial condition x0, we only need to enforce the final state constraint.
        ...
        ceq = x - xf_des;  % enforce final state
        c = [];
    end
