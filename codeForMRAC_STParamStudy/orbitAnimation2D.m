...
%         rSat = 1500; % Visible radius on plot (not to scale)
%         % Initial position
...
%     % addpoints(hTraj, x(end, 1), x(end, 2));
%     % if exist(chaserImgFile, 'file')
%     %     set(hSat, 'XData', x(end, 1) + [-rSat rSat], 'YData', x(end, 2) + [rSat -rSat]);
%     % else
