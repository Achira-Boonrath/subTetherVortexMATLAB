...
numMU  = 2;
l_0 = 28.28427125;              % Length parameter
m_H = 0.5 + m_net05;            % Mass H
m_P = 7 + m_net05/2;            % Mass P
F_T = numMU.*22;                 % Force parameter
%% Optimal Control Problem Setup
T = 3.3;         % total time horizon (s)
N = 40;          % number of collocation intervals (creates 61 knot points)
...
xc_des = 0;
xdotc_des = 0;
theta_des = pi/2;    % upright (target angle)
...
u_min = 0;   % max backward force (N)
u_max = pi/2;    % max forward force (N)

% State Variable Bounds (keep solution physically reasonable)
xc_min = -50;        xc_max = 50;              % cart position limits
xc_dot_min = -200;   xc_dot_max = 200;        % cart velocity limits
theta_min = -theta_des;  theta_max = theta_des;       % allow multiple rotations
...
R_u = 1e-3;  % penalty on control effort: \int (u^2)dt
Qf = diag([1e-3, 1e-3, 400, 1e-3]);  % terminal state penalty (only applied at t=T)
...
options = optimoptions('fmincon', ...
    ...
    'MaxIterations', 1500, ...          % allow up to 500 iterations
...
%% ============ NESTED FUNCTION DEFINITIONS ============
    ...
    function xd = cartpole_dynamics(x,u)
        ...
        % Assign variables for phase space
        phi = u;       % phi in degrees

        psi_dot = theta_dot;  % psi_dot (set to zero for this plot)
        psi = theta;       % psi in degrees

        % Compute intermediate quantities for the equations of motion
        A = -2 .* m_P .* l_0 .* psi_dot.^2 .* cos(psi) - 2 .* F_T .* cos(phi);
        B = 2 .* F_T .* l_0 .* (sin(phi) .* cos(psi) - cos(phi) .* sin(psi));
        % Compute the denominator for the accelerations
        common_den = l_0 .* (-2 .* m_P .* sin(psi).^2 + m_H + 2 .* m_P);
        % Compute the acceleration of y_H (not used in plot)
        yH_ddot = - (B .* sin(psi) - A .* l_0)./ common_den;
        % Compute the acceleration of psi (second derivative)
        psi_ddot = (B .* m_H + 2 .* B .* m_P - 2 .* A .* l_0 .* m_P .* sin(psi))./ ...
           (2 .* m_P .* l_0.* common_den);
        
        % Return state derivative as column vector
        xd = [xc_dot; yH_ddot; theta_dot; psi_ddot];
    end
