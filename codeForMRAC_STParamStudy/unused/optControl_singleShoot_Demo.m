% function optControl_singleShoot_Demo
...
tf = meetupTime;                % Final time (seconds) 
x0 = [0 -initialDistance 0 0 ]';           % Initial state: x (m), y (m), vx (m/s), vy (m/s)
...
z0 = [lambda0; x0];
[t, z] = ode23(@(t, z) hamiltonian_odeConstT(t, z), [0 tf], z0);
...
function ds = hamiltonian_odeConstT(t, s)

L = s(1:4);               % states (columns correspond to [x y vx vy])
x = s(5:end);            % costates (columns correspond to [L(1) L(2) L(3) L(4)])
...
ds(5) =                           x(3); 
ds(6) =                           x(4); 
ds(7) = (3*x(1))/1000000 - L(3) + x(4)/500; 
ds(8) =                - L(4) - x(3)/500; 

end

function F = shootingConstT(lambda0,x0,xf,tf)

    z0 = [lambda0; x0];

    [~,z] = ode45(@(t,z) hamiltonian_odeConstT(t,z),[0 tf],z0);

    x_tf = z(end,end-length(x0)+1:end).';
    F = [x_tf(1:end) - xf(1:end)];  % terminal error
