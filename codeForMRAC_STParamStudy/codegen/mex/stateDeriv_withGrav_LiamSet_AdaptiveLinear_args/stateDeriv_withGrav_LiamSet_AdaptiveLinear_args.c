 ...
 *
 * stateDeriv_withGrav_LiamSet_AdaptiveLinear_args.c
 *
...
static emlrtRSInfo emlrtRSI = {
    284,                                               /* lineNo */
    "stateDeriv_withGrav_LiamSet_AdaptiveLinear_args", /* fcnName */
    "D:\\CM "
    "Labs\\subTetherVortexMATLAB\\codeForMRAC_STParamStudy\\stateDeriv_"
    "withGrav_LiamSet_AdaptiveLinear_args.m" /* pathName */
};
                                                     ...
                                                     real_T ds[39])
{
  ...
  Phi_idx_0 = 0.5 * -s[22];
  Qmat_desCh[8] = Phi_idx_0;
  l_mt = 0.5 * s[19];
  Qmat_desCh[1] = l_mt;
  Qmat_desCh[5] = Phi_idx_0;
  ...
  delta = s[8] * s[8];
  x1 = s[9] * s[9];
  rotMat_C_A_I[0] = ((Tmag_mt + Qmat_desCh_tmp) - delta) - x1;
  Phi_idx_0 = s[7] * s[8];
  l_mt = s[6] * s[9];
  rotMat_C_A_I[1] = 2.0 * (Phi_idx_0 - l_mt);
  x1dot = s[7] * s[9];
  desElonVary = s[6] * s[8];
  rotMat_C_A_I[2] = 2.0 * (x1dot + desElonVary);
  rotMat_C_A_I[3] = 2.0 * (Phi_idx_0 + l_mt);
  Tmag_mt -= Qmat_desCh_tmp;
  rotMat_C_A_I[4] = (Tmag_mt + delta) - x1;
  Qmat_desCh_tmp = s[8] * s[9];
  Phi_idx_0 = s[6] * s[7];
  rotMat_C_A_I[5] = 2.0 * (Qmat_desCh_tmp - Phi_idx_0);
  rotMat_C_A_I[6] = 2.0 * (x1dot - desElonVary);
  rotMat_C_A_I[7] = 2.0 * (Qmat_desCh_tmp + Phi_idx_0);
  rotMat_C_A_I[8] = (Tmag_mt - delta) + x1;
  Tmag_mt = s[19] * s[19];
  Qmat_desCh_tmp = s[20] * s[20];
  delta = s[21] * s[21];
  x1 = s[22] * s[22];
  rotMat_D_A_I[0] = ((Tmag_mt + Qmat_desCh_tmp) - delta) - x1;
  Phi_idx_0 = s[20] * s[21];
  l_mt = s[19] * s[22];
  rotMat_D_A_I[1] = 2.0 * (Phi_idx_0 - l_mt);
  x1dot = s[20] * s[22];
  desElonVary = s[19] * s[21];
  rotMat_D_A_I[2] = 2.0 * (x1dot + desElonVary);
  rotMat_D_A_I[3] = 2.0 * (Phi_idx_0 + l_mt);
  Tmag_mt -= Qmat_desCh_tmp;
  rotMat_D_A_I[4] = (Tmag_mt + delta) - x1;
  Qmat_desCh_tmp = s[21] * s[22];
  Phi_idx_0 = s[19] * s[20];
  rotMat_D_A_I[5] = 2.0 * (Qmat_desCh_tmp - Phi_idx_0);
  rotMat_D_A_I[6] = 2.0 * (x1dot - desElonVary);
  rotMat_D_A_I[7] = 2.0 * (Qmat_desCh_tmp + Phi_idx_0);
  rotMat_D_A_I[8] = (Tmag_mt - delta) + x1;
  /*  N2L Chaser */
  ...
  r = _mm_mul_pd(r, _mm_set1_pd(Phi_idx_0));
  r = _mm_add_pd(r3, r);
  r3 = _mm_loadu_pd(&s[0]);
  r = _mm_add_pd(r3, r);
  ...
  Phi_idx_0 = distAttPt_to_C[1] * s[10] - distAttPt_to_C[0] * s[11];
  ...
  desElonVary = args->x1_m0 +
                muDoubleScalarMin(
                    muDoubleScalarMax(
                        (t - args->tOnChaser_fromt0) / args->slopeTime, 0.0),
                    1.0) *
  ...
  l_mt = args->cVec[0] / args->chaserM * s[33] +
         args->Kvec[0] / args->chaserM * Tmag_mt;
  if (!(l_mt > 0.0)) {
    l_mt = 0.0;
  }
  ds[33] = -l_mt + muDoubleScalarMin(
                       muDoubleScalarMax(
                           args->Kp * (desElonVary - s[32]) + args->Kd * -s[33],
  ...
  /*  ========================== */
  /*  (2x1) */
  Phi_idx_0 = x1 * ((real_T)maskMTreal - 1.0);
  l_mt = x1dot * ((real_T)maskMTreal - 1.0);
  /*  (2x1) */
  /*  ========================== */
  /*  Control input */
  /*  ========================== */
  delta = ((s[35] * x1 + s[36] * x1dot) + s[34] * desElonVary) -
          (s[37] * Phi_idx_0 + s[38] * l_mt);
  /*  ========================== */
  /*  Adaptive laws */
  /*  ========================== */
  /*  gamma = 45; ThrustSaturation = 825; */
  /*  Gamma_x = chaserM * 500000 * gamma * [[400 ,   0];       [  0 ,
   * 1/Kvec(1)]]; */
  /*  Gamma_r = chaserM * 7000000 * gamma * 1; */
  /*  Gamma_theta = chaserM * 500000 * gamma * [[10 ,   0];       [  0 ,
   * 1/Kvec(1)]]; */
  ...
  memset(&b_x1[0], 0, sizeof(real_T) << 1);
  Tmag_mt = ((b_x1[0] + Tmag_mt * args->P[0]) + Qmat_desCh_tmp * args->P[1]) *
                args->B_linear[0] +
            ((b_x1[1] + Tmag_mt * args->P[2]) + Qmat_desCh_tmp * args->P[3]) *
                args->B_linear[1];
  Qmat_desCh_tmp = args->sigmaMRACLin * muDoubleScalarAbs(Tmag_mt);
  /*  ========================== */
  /*  Thrust along +cZ direction */
  /*  ========================== */
  a = muDoubleScalarMin(muDoubleScalarMax(delta, -args->ThrustSaturation),
                        args->ThrustSaturation);
  maskFT = (delta * delta < args->ThrustSaturation * args->ThrustSaturation);
  ds[34] = (-args->Gamma_r * desElonVary * Tmag_mt -
            Qmat_desCh_tmp * args->Gamma_r * desElonVary * s[34]) *
           (real_T)maskFT;
  r4 = _mm_set1_pd(-1.0);
  _mm_storeu_pd(&quat_desCh[0],
                _mm_mul_pd(_mm_loadu_pd(&args->Gamma_x[0]), r4));
  _mm_storeu_pd(&quat_desCh[2],
                _mm_mul_pd(_mm_loadu_pd(&args->Gamma_x[2]), r4));
  memset(&b_x1[0], 0, sizeof(real_T) << 1);
  r = _mm_loadu_pd(&quat_desCh[0]);
  r3 = _mm_loadu_pd(&b_x1[0]);
  _mm_storeu_pd(&b_x1[0], _mm_add_pd(r3, _mm_mul_pd(r, _mm_set1_pd(x1))));
  r = _mm_loadu_pd(&quat_desCh[2]);
  r3 = _mm_loadu_pd(&b_x1[0]);
  _mm_storeu_pd(&b_x1[0], _mm_add_pd(r3, _mm_mul_pd(r, _mm_set1_pd(x1dot))));
  ds[35] = (b_x1[0] * Tmag_mt - (Qmat_desCh_tmp * args->Gamma_x[0] * s[35] +
                                 Qmat_desCh_tmp * args->Gamma_x[2] * s[36])) *
           (real_T)maskFT;
  ds[36] = (b_x1[1] * Tmag_mt - (Qmat_desCh_tmp * args->Gamma_x[1] * s[35] +
                                 Qmat_desCh_tmp * args->Gamma_x[3] * s[36])) *
           (real_T)maskFT;
  _mm_storeu_pd(&quat_desCh[0],
                _mm_mul_pd(_mm_loadu_pd(&args->Gamma_theta[0]), r4));
  _mm_storeu_pd(&quat_desCh[2],
                _mm_mul_pd(_mm_loadu_pd(&args->Gamma_theta[2]), r4));
  memset(&b_x1[0], 0, sizeof(real_T) << 1);
  r = _mm_loadu_pd(&quat_desCh[0]);
  r3 = _mm_loadu_pd(&b_x1[0]);
  _mm_storeu_pd(&b_x1[0],
                _mm_add_pd(r3, _mm_mul_pd(r, _mm_set1_pd(Phi_idx_0))));
  r = _mm_loadu_pd(&quat_desCh[2]);
  r3 = _mm_loadu_pd(&b_x1[0]);
  _mm_storeu_pd(&b_x1[0], _mm_add_pd(r3, _mm_mul_pd(r, _mm_set1_pd(l_mt))));
  ds[37] =
      (b_x1[0] * Tmag_mt - (Qmat_desCh_tmp * args->Gamma_theta[0] * s[37] +
                            Qmat_desCh_tmp * args->Gamma_theta[2] * s[38])) *
      (real_T)maskFT;
  ds[38] =
      (b_x1[1] * Tmag_mt - (Qmat_desCh_tmp * args->Gamma_theta[1] * s[37] +
                            Qmat_desCh_tmp * args->Gamma_theta[3] * s[38])) *
      (real_T)maskFT;
  /*     %% PID */
  /*      errorInt = s(33); */
  ...
  /*     %% with gravity; */
  Phi_idx_0 = b_norm(&s[0]);
  l_mt = -args->mu * args->chaserM;
  delta = muDoubleScalarPower(Phi_idx_0, 3.0);
  x1 = s[2] / Phi_idx_0;
  Tmag_mt = args->J2on * args->chaserM *
            (0.001623945 * args->mu *
             (4.0678884E+13 / muDoubleScalarPower(Phi_idx_0, 4.0)));
  ...
  accChaser[2] += y_tmp[8];
  Qmat_desCh_tmp = 5.0 * (x1 * x1);
  VR_mt[0] = Tmag_mt * ((Qmat_desCh_tmp - 1.0) * (s[0] / Phi_idx_0));
  VR_mt[1] = Tmag_mt * ((Qmat_desCh_tmp - 1.0) * (s[1] / Phi_idx_0));
  VR_mt[2] = Tmag_mt * ((Qmat_desCh_tmp - 3.0) * x1);
  /*     %% Sliding Att Control */
  /*  Vortex */
  /*      k_Loc = -evec_mt; */
  Tmag_mt = b_norm(&s[3]);
  ...
  _mm_storeu_pd(
      &accChaser[0],
      _mm_div_pd(
          _mm_add_pd(_mm_add_pd(_mm_add_pd(r3, _mm_mul_pd(_mm_set1_pd(a), r)),
                                _mm_div_pd(_mm_mul_pd(_mm_set1_pd(l_mt), r2),
                                           _mm_set1_pd(delta))),
                     r5),
          _mm_set1_pd(args->chaserM)));
  r = _mm_loadu_pd(&s[3]);
  _mm_storeu_pd(&l_mt_vec[0],
                _mm_mul_pd(_mm_div_pd(r, _mm_set1_pd(Tmag_mt)), r4));
  _mm_storeu_pd(&b_b[0], _mm_div_pd(r2, _mm_set1_pd(Phi_idx_0)));
  accChaser[2] =
      (((Tvec_mt[2] + a * accChaser[2]) + l_mt * s[2] / delta) + VR_mt[2]) /
      args->chaserM;
  l_mt_vec[2] = -(s[5] / Tmag_mt);
  VR_mt[0] = l_mt_vec[1] * x1 - b_b[1] * l_mt_vec[2];
  VR_mt[1] = b_b[0] * l_mt_vec[2] - l_mt_vec[0] * x1;
  ...
  b_x1[0] = muDoubleScalarMin(muDoubleScalarMax(dv[0], -1.0), 1.0);
  b_x1[1] = muDoubleScalarMin(muDoubleScalarMax(dv[1], -1.0), 1.0);
  r = _mm_loadu_pd(&b_x1[0]);
  ...
  for (i = 0; i < 4; i++) {
    ...
    Phi_idx_0 = b_norm(l_mt_vec);
    Tmag_mt = b_distAttPt_to_D[2] * delta - 0.0 * x1;
    Qmat_desCh_tmp = b_distAttPt_to_D[0] * x1 - b_distAttPt_to_D[2] * x1dot;
    l_mt = x1dot * 0.0 - b_distAttPt_to_D[0] * delta;
    r = _mm_loadu_pd(&l_mt_vec[0]);
    r = _mm_div_pd(r, _mm_set1_pd(Phi_idx_0));
    _mm_storeu_pd(&l_mt_vec[0], r);
    r = _mm_loadu_pd(&s[29]);
    ...
    appTorqueTarget[2] =
        Phi_idx_0 + (b_distAttPt_to_D[0] * l_mt_vec[1] - l_mt_vec[0] * 0.0);
  ...
  Phi_idx_0 = muDoubleScalarPower(b_norm(&s[26]), 3.0);
  /*  */
  memset(&l_mt_vec[0], 0, 3U * sizeof(real_T));
  ...
  for (b_i = 0; b_i < 3; b_i++) {
    int32_T y_tmp_tmp;
    accPoint1[b_i] =
        (-(accPoint1[b_i] + Tvec_mt[b_i]) + Tmag_mt * s[b_i + 26] / Phi_idx_0) /
        args->massPoint1;
    maskMTreal = b_i << 2;
    for (i = 0; i < 3; i++) {
  ...
  for (i = 0; i < 3; i++) {
    VR_mt[i] =
        (desElonVary * (distAttPt_to_D[i] * 0.0 -
                        ((y_tmp[i] * Tmag_mt + y_tmp[i + 3] * Qmat_desCh_tmp) +
                         y_tmp[i + 6] * Phi_idx_0)) +
         slidingVec_tmp[i]) -
        (((real_T)b_a[i] * l_mt + (real_T)b_a[i + 3] * delta) +
         (real_T)b_a[i + 6] * x1);
  }
  Tvec_mt[0] =
...
/* End of code generation (stateDeriv_withGrav_LiamSet_AdaptiveLinear_args.c) */
