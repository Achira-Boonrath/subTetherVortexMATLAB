function [t, x, lambda, u] = orbitTransferData(at, theta_f, r0)
    close all; 
    %% 
    ...
    % chem prop
    % lb = -1.0e-4*ones(nvars,1);
    % ub =  1.0e-4*ones(nvars,1);
    % % 
    % % % electric prop
    % % % lb = -[1e-0*ones(nvars-1,1)', 0]'; % Lower bounds (adjust as needed)
    % % % ub =  [1e-0*ones(nvars-1,1)', pi/2]'; % Upper bounds (adjust as needed)
    % % % opts = optimoptions('particleswarm', 'Display', 'iter', "SwarmSize", 100, 'MaxIterations', 90); %, "UseParallel", true);
    % opts = optimoptions('particleswarm', "SwarmSize", 200, 'MaxIterations', 60); %, "UseParallel", true);
    ...
    lambda0 = fsolve(@(lam0) shootingConstT(lam0, x0, xf, tf, muEarth, Tmax, Isp, g0, epsilon), ...
        lambda0_guess, ...
        optimoptions('fsolve', 'Display', 'none', 'MaxIterations', 10));
        % optimoptions('fsolve'));
        % optimoptions('fsolve', 'Display', 'iter', 'MaxIterations', 90));
    
    %% Integrate the Hamiltonian system with the solved initial costates -- actual optimal
    ...
    %% Integrate the Hamiltonian system with the solved initial costates -- Hohman
